                ...
                *peses[i]=AGUA;                                                     //que me ponga agua
                ...
                hasta -=1;
                 ...
                 if(q<cant_pes){								//SI SE MUEVE POR PRIMERA VEZ
                    while (i < hasta){		            
                        valor3[i] = *(peses[i]-1);               //guardo el valor para salvar lo que valia antes de que caiga el pez               
                        if(valor3[i]==MARIO){       
                            vida-=1;
#ifdef RASPI
                            printf("Perdiste una vida , te quedan:%d\n",vida);
                        ...
                        *(peses[i]) = AGUA;             //cuando se movio por primera vez, que me ponga agua donde arranco el PEZ
                        *(peses[i]-1) = PES;            //muevo el PEZ para la izquierda     
                       
                        q++;                                            //todo es para que avance un pez, aumento i para que vaya al otro pez

                        peses [i] -= 1;					//la nueva direeccion del pez es la de la izquuierda
                        i++;
                    }
                    sleep(VELOCIDAD_PES);             //velocidad de los peZes
                }				
                else{
                    while (i < hasta){		        
...
void * enemigo_pulpo(){                 //thread que controla los movimientos del pulpo
    while(1){
        ...
        while ((nivel_var==nivel)&&(pulpo)){       //hasta que cambie de nivel, si cambia, vuelve a arrancar con nivel_var con otro numerp
            while (stop){               //para la pausa
            }
            ...
            if(reinicio==1){
                ...
                if(primera_vez==0){
                    for(k=0;(k<MOV_PULPO-1)&&(reboot==0);k++){				//MOVIMIENTO DEL PULPO PARA ARRIBA
                        if(reinicio==1){
                            pulpo=0;
                            reboot=1;
                        }
                ...
                for(k=0;(k<MOV_PULPO-1)&&(reboot==0);k++){				//MOVIMIENTO DEL PULPO PARA ABAJO
                    if(reinicio==1){
                        pulpo=0;
                        reboot=1;
                    }
